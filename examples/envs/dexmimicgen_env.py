...
class DexMimicGenGymEnv(gym.Env):
    ...
    def __init__(
        ...
        camera_heights: Optional[List[int]] = None,
        camera_widths: Optional[List[int]] = None,
        im_size: int = 256,
        seed: int = 1234,
        ...
        **env_kwargs
        ...
        camera_heights = camera_heights or [256, 128]
        camera_widths = camera_widths or [256, 128]
        ...
        self.observation_space = gym.spaces.Dict({
            "image_primary": gym.spaces.Box(
                low=0, high=255, shape=(im_size, im_size, 3), dtype=np.uint8
            ),
    ...
    def _convert_obs_to_octo_format(self, obs):
        ...
        for key in ["image_primary", "image_wrist"]:
            if key in octo_obs:
                image = octo_obs[key]
                if image.shape[0] != self.im_size or image.shape[1] != self.im_size:
                    # Simple resize - in practice you might want more sophisticated resizing
                    import cv2
                    octo_obs[key] = cv2.resize(image, (self.im_size, self.im_size))
